...
{ //显示QModelIndex的行、列号
    ui->LabInfo->setText(QString::asprintf("row = %d, column = %d", index.row(), index.column()));
}
...
{// 显示数据模型的StringList
    QStringList tempList = theModel->stringList();  //获取数据模型的StringList

    ui->plainTextEdit->clear(); //文本框清空

    for (int i = 0; i < tempList.size(); i++)
    {
        ui->plainTextEdit->appendPlainText(tempList.at(i));//显示数据模型的StringList()返回的内容
    }
}
