...
{ //初始化Tree
    QString data = "";  // Item的Data 存储的string

    ui->treeFiles->clear(); //清除目录树所有节点

    QIcon icon(":/images/icons/15.ico");    //设置ICON的图标为15.ico

    QTreeWidgetItem * item = new QTreeWidgetItem(itTopItem);    //新建节点时设定类型为 itTopItem
    ...
    item->setData(MainWindow::colItem, Qt::UserRole, QVariant(data));   //设置节点第1列的Qt::UserRole的Data
...
{//添加一个目录节点
    QIcon icon(":/images/icons/open3.bmp"); //设置ICON的图标为open3.bmp

    QString NodeText = getFinalFolderName(dirName); //从一个完整目录名称里，获得最后的文件夹名称

    QTreeWidgetItem * item;//节点

    item = new QTreeWidgetItem(itGroupItem);//新建节点, 设定type为 itGroupItem

    item->setIcon(MainWindow::colItem, icon);    //设置图标

    item->setText(MainWindow::colItem, NodeText);   //最后的文件夹名称，第1列

    item->setText(MainWindow::colItemType, "type=itGroupitem"); //完整目录名称，第2列

    item->setFlags(Qt::ItemIsSelectable | Qt::ItemIsUserCheckable | Qt::ItemIsEnabled | Qt::ItemIsAutoTristate);    //设置节点选项

    item->setCheckState(MainWindow::colItem, Qt::Checked);  //节点选中

    item->setData(MainWindow::colItem, Qt::UserRole, QVariant(dirName)); //设置角色为Qt::UserRole的Data,存储完整目录名称
...
{//添加一个图片文件节点
    QIcon icon(":/images/icons/31.ico"); //设置ICON的图标为31.ico

    QString NodeText = getFinalFolderName(aFilename); //从一个完整目录名称里，获得最后的文件夹名称

    QTreeWidgetItem * item;//节点

    item = new QTreeWidgetItem(itImageItem);//新建节点, 设定type为 itGroupItem

    item->setIcon(MainWindow::colItem, icon);    //设置图标

    item->setText(MainWindow::colItem, NodeText);   //最后的文件夹名称，第1列

    item->setText(MainWindow::colItemType, "type=itImageItem"); //完整目录名称，第2列

    item->setFlags(Qt::ItemIsSelectable | Qt::ItemIsUserCheckable | Qt::ItemIsEnabled | Qt::ItemIsAutoTristate);    //设置节点选项

    item->setCheckState(MainWindow::colItem, Qt::Checked);  //节点选中

    item->setData(MainWindow::colItem, Qt::UserRole, QVariant(aFilename)); //设置角色为Qt::UserRole的Data,存储完整目录名称
...
{// 选择一个文件夹,作为当前节点的子节点加入
    ...
    if (!dir.isEmpty())//选择目录名称不为空
    {
        ...
        if (parItem == NULL)
        {//避免刚打开文件时由于未选取当前节点，导致的parItem指针为空的错误
            QMessageBox::critical(NULL, "错误", "未选择节点", QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
...
{//添加图片文件节点
    ...
    if (item == NULL)
    {//避免刚打开文件时由于未选取当前节点，导致的item指针为空的错误
        QMessageBox::critical(NULL, "错误", "未选择节点", QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
    ...
    {
        parItem = item; //若当前节点不是图片节点，取当前节点为父节点
    }
    else
    {
        parItem = item->parent();  //否则取其父节点作为父节点
    }
