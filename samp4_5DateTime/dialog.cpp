    ...
    ui(new Ui::Dialog)
{
    ...
    QObject::connect(fTimer, SIGNAL(timeout()), this, SLOT(on_timer_timeout())); //关联定时器的信号与槽
}
...
{ //定时器中断响应槽函数
    ...
    int val = ui->progressBar->value(); //读取progressBar的数值
    ...
    ui->progressBar->setValue(val);  //设置progressBar的数值
}
...
{ //获取当前日期时间，为三个专用编辑器设置日期时间数据，并转换为字符串在LineEdit里显示
    ...
    ui->editTime->setText(curDateTime.toString("hh点mm分ss秒")); //转换为字符串显示
    ...
    ui->editDateTime->setText(curDateTime.toString("yyyy/mm/dd hh点mm分ss秒"));   //转换为字符串显示
}
